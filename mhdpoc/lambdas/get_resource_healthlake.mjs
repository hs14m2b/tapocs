...
export const getResourceWithRetry = async (maxDuration, resourceId, resourceType, org, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM) =>
  {
    ...
    let retrieveResult = await getResource(resourceId, resourceType, org, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM);
    let httpResponse = retrieveResult.status;
    while (true){
      //if no entry values....
      if (httpResponse > 299)
      {
        ...
        retrieveResult = await getResource(resourceId, resourceType, org, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM);
        httpResponse = retrieveResult.status;
      }
      else{
        console.log("found resource from healthlake");
        break;
      }
    }
...
export const getResource = async (resourceId, resourceType, org, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM) =>
{
