...
Home.getInitialProps = async (ctx) => {
  ...
  //ctx.query includes slotb64, servicerequestb64, servicerequestId, barsidentifier, barsserviceid
  if (ctx.req) {
    ...
    try {
      let apiResult = await getResult("https://main-mabr8-barspocui-nextjsfe.nhsdta.com/extapi/booknewappointment", new URLSearchParams(ctx.query).toString(),29000);
      props['result'] = (apiResult) ? true: false;
      props.retrievedResult = true;
      props['barsResponse'] = apiResult;
      props['barsidentifier'] = apiResult.id;
    } catch (error) {
...
//call the API to get the result
  ...
  while (!retrievedResult)
  {
    console.log("in loop number " + loopNo);
    try {
      let apiResult = await getResult("https://main-mabr8-barspocui-nextjsfe.nhsdta.com/extapi/booknewappointment", new URLSearchParams(ctx.query).toString(), 20000 + (loopNo * 1000));
      retrievedResult = true;
      props['result'] = (apiResult) ? true: false;
      props.retrievedResult = true;
      props['barsResponse'] = apiResult;
