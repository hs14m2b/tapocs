...
function Home(props) {
  ...
  function checkData(props) {
    ...
    let barsidentifier=props.barsidentifier; //this will be ServiceRequest/{id}
    let barsserviceid=props.barsserviceid;
    let healthcareServiceId=props.barsResponse.performer.find((performer) => performer.type == "HealthcareService").reference;
    let servicerequestId=props.barsResponse.id;
    let servicerequestb64=encodeURIComponent(Buffer.from(JSON.stringify(props.barsResponse)).toString('base64'));
    router.push("/getslotsnewappt?barsidentifier=" + barsidentifier + "&barsserviceid=" + barsserviceid + "&healthcareServiceId=" + healthcareServiceId + "&servicerequestId=" + servicerequestId + "&servicerequestb64=" + servicerequestb64);
  }
  ...
  return (
    <>
      ...
      <h2 className="nhsuk-heading-l">
        This page invokes the BaRS API to retrieve a single specified service request for a patient
      </h2>
      ...
      <div id="noMessage" className={(props.retrievedResult && ! props.result) ? "nhsuk-error-summary" : "nhsuk-error-summary nhsuk-hidden"} aria-labelledby="error-summary-title" role="alert" tabIndex="-1">
        ...
        <div className="nhsuk-error-summary__body">
          <p class="nhsuk-u-font-size-32">
            Unable to retrieve service request {props.barsidentifier} for patient {props.nhsnumber}
          </p>
      ...
      <div id="yesMessage" className={(props.retrievedResult && props.result) ? "" : "nhsuk-hidden"}>
        <p className="nhsuk-u-font-size-32">
          Yes, retrieved service request details for {props.nhsnumber}!
        </p>
        ...
        <dl class="nhsuk-summary-list">
          ...
          <div class="nhsuk-summary-list__row">
            ...
            <dd class="nhsuk-summary-list__value">
              { new Date(props.barsResponse.occurrencePeriod.start).toLocaleDateString() } to { new Date(props.barsResponse.occurrencePeriod.end).toLocaleDateString() }
            </dd>
          </div>
          ...
          <div class="nhsuk-summary-list__row">
            ...
            <dd class="nhsuk-summary-list__value">
              {props.barsResponse.performer.find((performer) => performer.type == "HealthcareService").display}
            </dd>
          ...
          <div class="nhsuk-summary-list__row">
            <dt class="nhsuk-summary-list__key">
              Book an appointment for this service request
            </dt>
            <dd class="nhsuk-summary-list__value">
      ...
      <Spinner message="Retrieving Available Service Request Slots"></Spinner>
...
Home.getInitialProps = async (ctx) => {
  ...
  if (ctx.req) {
    ...
    try {
      let apiResult = await getResult("https://main-mabr8-barspocui-nextjsfe.nhsdta.com/extapi/getservicerequest", new URLSearchParams(ctx.query).toString(),20000);
...
//call the API to get the result
  ...
  while (!retrievedResult)
  {
    console.log("in loop number " + loopNo);
    try {
      let apiResult = await getResult("https://main-mabr8-barspocui-nextjsfe.nhsdta.com/extapi/getservicerequest", new URLSearchParams(ctx.query).toString(), 10000 + (loopNo * 1000));
      retrievedResult = true;
      props.retrievedResult = true;
      props['barsResponse'] = apiResult;
      //check that barsResponse.resourceType is ServiceRequest
