...
const findDocRef = async (nhsnumber, odscode, identity_token, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM) =>
{
  ...
  let url = HTTPS + APIENVIRONMENT + ".api.service.nhs.uk" + "/patient-data-manager/FHIR/R4/DocumentReference?" + subjectIdentifier;
  ...
  if (environmentNeedsAuth(APIENVIRONMENT)){
    ...
    if (!ACCESSTOKEN || ((currentTime - ACCESSTOKENLASTRETRIEVED) > 240000)){
      ...
      //need client to be enabled for PDM via NHS login before the next line will work
...
export class find_document_ref_pdm{
  constructor(){
