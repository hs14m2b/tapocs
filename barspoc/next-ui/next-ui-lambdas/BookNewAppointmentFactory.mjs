import { handler as processor } from './BookNewAppointmentProcessor.mjs';
...
export const handler = async (event) => {
  console.log(JSON.stringify(event));
    try {
        let sendAppointmentBarsObject = new put_appointment_bars();
        return await processor(event, sendAppointmentBarsObject, getParameterCaseInsensitive, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM)
    } catch (error) {
