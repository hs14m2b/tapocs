...
describe("ServiceRequest Create Processor", function() {
  ...
  it("can create a service request", async function() {
    ...
    let response = await handler(event, create_resource_fhir_instance, update_resource_healthlake_instance, snsCommonFunctionHelperObjectInstance, getParameterCaseInsensitive, process.env['APIKEYSECRET'], process.env['APIENVIRONMENT'], process.env['APIKNAMEPARAM'], true, "SNSTOPICARN");
    console.log(JSON.stringify(response));
    expect(response.statusCode).toEqual(201);
  });

  it("triggers SNS when creating task fails", async function() {
    let event = {
      "headers": {
        "content-encoding": "NOTGZIP",
        "nhsd-end-user-organisation-ods": "X26",
    ...
    let snsr1 = { "MessageId": "abc123" };
    let pdmTask = { resourceType:"Task", id: "task-1" };
    let create_resource_fhir_instance = new create_resource_fhir([serviceRequest, false], []);
    let update_resource_healthlake_instance = new update_resource_healthlake([], []);
    let snsCommonFunctionHelperObjectInstance = new snsCommonFunctionHelperObject([snsr1]);
    console.log("helper object is " + JSON.stringify(create_resource_fhir_instance));
    let response = await handler(event, create_resource_fhir_instance, update_resource_healthlake_instance, snsCommonFunctionHelperObjectInstance, getParameterCaseInsensitive, process.env['APIKEYSECRET'], process.env['APIENVIRONMENT'], process.env['APIKNAMEPARAM'], true, "SNSTOPICARN");
