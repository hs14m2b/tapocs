...
const updateDocRef = async (docRef, APIENVIRONMENT, APIKEYSECRET, APIKNAMEPARAM) =>
{
  let postString = JSON.stringify(docRef);
  console.log(postString);
  let XRequestID = uuidv4();
  let org = docRef.custodian.identifier.value;
  let url = HTTPS + APIENVIRONMENT + ".api.service.nhs.uk" + "/booking-and-referral/FHIR/R4/DocumentReference/" + docRef.id;
...
export class put_document_ref_bars{
  constructor(){
    this.updateDocRef = updateDocRef;
  }
}
