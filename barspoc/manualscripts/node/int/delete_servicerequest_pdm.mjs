...
let id = "f91f4be2-d658-3fe4-8c85-50cb40739db4"; // replace with the actual ServiceRequest ID

async function deleteServiceRequest (versionId, accessToken)
  {
    //format http header value of If-Match to be "W/\"1\""
    let ifMatch = "W/\"" + versionId + "\"";
    console.log("ifMatch is " + ifMatch);
    let url = HTTPS + APIDomain + "/patient-data-manager/FHIR/R4/ServiceRequest/" + id;
    let options = {
    method: 'DELETE',
    headers: {
...
result = await deleteServiceRequest(versionId, accessToken);
