...
import { APIDomain, OAuthAPIKey, OAuthAPIKeyName, ODSCode, NHSDEndUserOrganisation, NHSDTargetIdentifier, NHSNumber } from './config.mjs';
  ...
  async function fetchAppointments (access_token)
  {
    let XRequestID = uuidv4();
    let patientIdentifier = new URLSearchParams({"patient:identifier" : "https://fhir.nhs.uk/Id/nhs-number|" + NHSNumber}).toString();
    let url = HTTPS + APIDomain + "/booking-and-referral/FHIR/R4/Appointment?" + patientIdentifier;
    // request option
    let options = {
      method: 'GET',
      headers: {
...
//for each entry in result, delete the appointment
for (let entry of result.entry) {
  let url = HTTPS + APIDomain + "/patient-data-manager/FHIR/R4/Appointment/" + entry.resource.id;
  let options = {
    method: 'DELETE',
    headers: {
      'Authorization': 'Bearer '+ access_token,
  ...
  console.log("request options are  " + JSON.stringify(options));
  console.log("url is " + url);
  console.log("deleting appointment with id " + entry.resource.id);
  let fetchResponse = await fetch(url, options);
  if (!fetchResponse.ok) {
    console.log(fetchResponse.status);
