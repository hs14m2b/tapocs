    ...
    if (!fetchResponse.ok) {
      ...
      if (responseJson.total > 0) {
        for (let i = 0; i < responseJson.total; i++) {
          ...
          await deleteDocRef(accessToken, responseJson.entry[i].resource.id, responseJson.entry[i].resource.custodian.identifier.value);
        }
    }
  }
...
async function deleteDocRef (accessToken, docRefId, custodian)
  {
