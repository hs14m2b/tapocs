...
import processMessageBundle from '../serviceRequestBundle.json' with {type: 'json'}
...
async function postBundle (access_token)
  {
    let XRequestID = uuidv4();
    processMessageBundle.id = XRequestID;
    let url = HTTPS + APIDomain + "/booking-and-referral/FHIR/R4/$process-message";
    // request option
    let options = {
      method: 'POST',
      headers: {
...
let result = await postBundle(access_token);
